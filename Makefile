...
local-dev-install: build
	...
	rm -rf ~/.terraform.d/plugins/localhost/panzer1119/proxmox
	mkdir -p ~/.terraform.d/plugins/localhost/panzer1119/proxmox/$(MAJOR).$(MINOR).$(NEXT_MICRO)/$(KERNEL)_$(ARCH)/
	cp bin/terraform-provider-proxmox ~/.terraform.d/plugins/localhost/panzer1119/proxmox/$(MAJOR).$(MINOR).$(NEXT_MICRO)/$(KERNEL)_$(ARCH)/
